...
class TexturePool {
public:
    typedef typename std::vector<std::weak_ptr<eagle::Texture>>::size_type index_type;
public:
    ...
    std::weak_ptr<eagle::Texture>& operator[](const index_type& index) { return m_textures[index]; }

private:
    eagle::RenderingContext* m_context = nullptr;
    std::vector<std::weak_ptr<eagle::Texture>> m_textures;

};

class TextureHandle : public Asset<std::weak_ptr<eagle::Texture>, TexturePool> {
public:
    TextureHandle(TexturePool* pool, TexturePool::index_type index) : Asset<std::weak_ptr<eagle::Texture>, TexturePool>(pool, index) {}
};
