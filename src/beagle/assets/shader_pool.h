...
class ShaderPool {
public:
    typedef std::unordered_map<std::string, std::shared_ptr<eagle::Shader>> ShaderMap;
...
public:
    ...
    std::shared_ptr<eagle::Shader>& operator[](const index_type& index) { return m_shaders[index]; }
...
class ShaderHandle : public Asset<std::shared_ptr<eagle::Shader>, ShaderPool> {
public:
    ShaderHandle(ShaderPool* pool, const ShaderPool::index_type& index) : Asset<std::shared_ptr<eagle::Shader>, ShaderPool>(pool, index) {}
};
