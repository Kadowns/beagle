...
vec3 calculate_illumination_pbr(in Illumination illumination, in vec3 position, in vec3 viewPosition, in PBRInput pbr){
    vec3 result = vec3(0);
    vec3 view = normalize(viewPosition - position);

    for (uint i = 0; i < illumination.directionalLightCount; i++){
        result += calculate_directional_light_pbr(illumination.directionalLights[i], position, view, pbr);
    }
    for (uint i = 0; i < illumination.pointLightCount; i++){
        result += calculate_point_light_pbr(illumination.pointLights[i], position, view, pbr);
    }
